...
def remover_favoritos(nome_pais: str):
    registros = carregar_favoritos()

    for paises in registros:
        if nome_pais not in paises["nome"]:
            return False
        else:
            if nome_pais == paises["nome"]:
                registros.remove(paises)
                salvar_favoritos(registros)
                return True
